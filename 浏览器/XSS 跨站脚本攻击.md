...
```
第一种 反射型
场景：比如在一个网站上面，弹出来一个广告「性感荷官...」,用户这时候点击进去了，访问了恶意广告的服务器，服务器，这时候直接返回一个<script>带各种恶意操作的脚本<script>，然后浏览器获取到了返回的内容，就会像平时加载脚本一样加载这个新的script，接着就会执行里面的代码，然后这个网站就会受到攻击了。
...
```
注入的方式
	1.在HTML中，恶意内容以script标签注入，如反射性注入脚本。
	2.在标签的 href、src 等属性中，包含 javascript: 等可执行代码。
	3.在 style 属性和标签中，包含类似 background-image:url("javascript:..."); 的代码（新版本浏览器已经可以防范）。
	4.在 style 属性和标签中，包含类似 expression(...) 的 CSS 表达式代码（新版本浏览器已经可以防范）。
	5.在内联的 JavaScript 中，拼接的数据突破了原本的限制（字符串，变量，方法名等），如eval()、setTimeout()、setInterval() 可以拼接数据并被执行。
	6.在 onload、onerror、onclick 等事件中，注入不受控制代码。
