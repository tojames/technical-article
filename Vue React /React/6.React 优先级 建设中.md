...
function unstable_runWithPriority(priorityLevel, eventHandler) {
  // 如果不在调度优先级级别的话，赋值为 NormalPriority
  ...
  try {
    // eventHandler 即是绕了一大圈的 dispatchEvent
    return eventHandler();
  } finally {
...
const classComponentUpdater = {
 enqueueSetState(inst, payload, callback) {
    ...
    // 调度更新，从 render 阶段开始，这里会出现中断渲染等问题
...
export function requestUpdateLane(fiber: Fiber): Lane {
  ...
  // 比如是：97，根据调度优先级来获得，用于获取更新优先级，
  // 不同是因为避免冲突，可能获取到的值并不是想要的，不用老是取shdule中获取
...
`setState` 继续执行，当处理完更新对象后，就开始调度处理 `render逻辑`，由 `scheduleUpdateOnFiber`  方法开启。
