...
function FiberRootNode(containerInfo, tag, hydrate) {
  this.tag = tag; 
  this.hydrate = hydrate; // 定义浏览器渲染还是服务端渲染
...
function FiberNode(
  ...
  this.type = null; // 定义与fiber关联的功能或类。对于类组件，它指向构造函数，对于DOM元素，它指定HTML标记。
...
递归构建fiber树，`/packages/react-reconciler/src/ReactFiberBeginWork.old.js`。

- 创建fiber节点
- diff 算法，updateHostComponent 函数会调用reconcileChildren，给虚拟 DOM 打上 flag。

begin是怎么处理节点的，这些参数对它来说好像无法继续递归下去「current, unitOfWork, subtreeRenderLanes」。`beginWork` 是不断的往下找，和深度优先搜索的思想是一样的。
...
- 构建或更新DOM节点，
  ...
  - 更新过程中，会计算DOM节点的属性，一旦属性需要更新，会为DOM节点对应的workInProgress节点标记Update的flag。
...
> 回到 `performSyncWorkOnRoot` 方法里，`/packages/react-reconciler/src/ReactFiberWorkLoop.old.js`,到最后面会去调用 	`commitRoot`,开启 `commit` 阶段，整个Commit阶段是同步执行的，又分为三个小阶段「before mutation，mutation，layout 」。
...
- 处理blur和focus相关逻辑，提前获取这些状态方便更新完毕后恢复状态
- 类组件，调用getSnapshotBeforeUpdate
- 调度useEffect「useEffect 交给 Schedule 去调度，它将会异步调用」
...
- 操作DOM，遍历 effectList 根据 flags 的不同，进行增、删、更新操作，对 ref  进行解绑
- 类组件，调用componentWillUnmount；
- 函数组件，执行useEffect、useLayoutEffect的销毁函数。
...
处理 DOM 渲染完毕之后的收尾逻辑，读取组件的状态。

- 给Ref 赋值上 DOM 元素

- 类组件，调用生命周期componentDidMount和componentDidUpdate，调用setState的回调。
- 针对函数组件，调用useLayoutEffect钩子函数的回调「可能会堵塞渲染，useEffect 会在 Schedule 异步调用」。
