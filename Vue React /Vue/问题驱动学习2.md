...
```js
v-model 可以看成是 value+input方法 的语法糖 组件的v-model 就是value+input的语法糖，这仅仅是默认值是这样。
...
function transformModel (options, data: any) {
  ...
  const event = (options.model && options.model.event) || 'input' ;(data.attrs || (data.attrs = {}))[prop] = data.model.value 
